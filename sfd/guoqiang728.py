...
def get_driver_data():
    dr = dict()
    path = os.path.join('.', 'rawdata', 'driver_imgs_list.csv')
    print('Read drivers data')
    df = pd.read_csv(path)
    for item in df.as_matrix():
        # print item
        dr[item[2]] = item[0]
    return dr
...
def load_train(img_rows, img_cols, color_type=1):
    ...
    for j in range(10):
        ...
        path = os.path.join('.', 'rawdata', 'train',
                            'c' + str(j), '*.jpg')
...
def read_and_normalize_test_data(img_rows=224, img_cols=224, color_type=1):
    ...
    print(test_data.shape[0], 'other samples')
...
def run_cross_validation(nfolds=10, nb_epoch=10, split=0.2, modelStr=''):
    ...
    for train_drivers, test_drivers in kf:
        ...
        for id in train_drivers:
            for i1, j1 in enumerate(driver_id):
                if j1 == unique_drivers[id]:
                    tmp_train_data.append(list(train_data[i1]))
                    tmp_train_target.append(list(train_target[i1]))

        for id in test_drivers:
            for i1, j1 in enumerate(driver_id):
                if j1 == unique_drivers[id]:
                    tmp_test_data.append(list(train_data[i1]))
                    tmp_test_target.append(list(train_target[i1]))

        flatten_train_data = np.array(tmp_train_data).astype('float32')
        flatten_train_target = tmp_train_target

        flatten_test_data = np.array(tmp_test_data).astype('float32')
        flatten_test_target = tmp_test_target

        print "&" * 10
        print "flatten_train_data.shape", np.array(flatten_train_data).shape
        print "tmp_train_target", np.array(tmp_train_target).shape
        print "flatten_test_data.shape", np.array(flatten_test_data).shape

        model.fit(np.array(flatten_train_data), np.array(flatten_train_target),
